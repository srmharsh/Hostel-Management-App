...
//-------------------------------------------------------------------------------fetch room data
  Future<void> fetchRoomsData() async {
    try {
      ...
      final fetchedRooms = await controller.fetchData();
      allRooms = fetchedRooms;
      allRooms.sort((a, b) => a.roomNo.compareTo(b.roomNo));
      rooms = List.from(allRooms);
      
      // Apply current filter
      if (selctedFilter == "Vacant Rooms") {
  ...
  addRoom(
    ...
    try {
      ...
      final room = RoomModel(
        ...
        currentOccupancy: 0,
        price: int.parse(rentController.text.trim()).toDouble(),
        status: 'available',
        type: 'single',
        amenities: [],
      );
  ...
  editRoom(BuildContext context) async {
    try {
      ...
      final room = RoomModel(
          ...
          floor: 1,
          currentOccupancy: currentNoOfResidents!,
          price: int.parse(rentController.text.trim()).toDouble(),
          status: 'available',
          type: 'single',
          amenities: []);
